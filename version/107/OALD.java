...
public class OALD {
	...
	String getStatictic() {
		return title + "\nВсего прочитано/ошибок: " + cntRead + "/" + cntErr
				+ " Пропущено: " + cntSkip + " Скопировано: " + cntSel;
	}
	...
	*/
	...
	void parse(String fullName, String fn) throws Exception {
		try {
			...
			display("print=" + entryW.printPosList);
			...
			if (Utils.isBlank(entryW.error)) {
				...
				if (entryW.isThe()) {
					if (entryW.inTypes1(Entry.TYPEC)) writeEntry(entryW, Entry.TYPEC
							+ " + " + Entry.THETHE);//departed
					else if (entryW.inTypes1(Entry.TYPEU)) writeEntry(entryW, Entry.TYPEU
							+ " + " + Entry.THETHE);
					else if (entryW.inTypes1(Entry.TYPESING)) writeEntry(entryW,
							Entry.TYPESING + " + " + Entry.THETHE); //absurd
					else if (entryW.inTypes1(Entry.TYPEPL)) writeEntry(entryW,
							Entry.TYPEPL + " + " + Entry.THETHE);//ancients
	...
	public static void main(String[] args) {
		...
		String srcFolder = "c:\\1\\dict\\oxford\\xml.full\\";
		if (DEBUG) srcFolder = "c:\\1\\dict\\oxford\\xml.tst\\";
