...
public class Entry {
	 ...
	 * @param hg
	 * @param pos
	 *            в какой части речи (noun, adj) ищем NOUN
	 * @return true - найден NOUN в pos false - не найден
	 * @throws IOException
	 */
	public boolean analyzePG0(Element entry, Element hg, String pos)
			throws IOException {
		OALD.display("analyzePG0(" + pos + ")");
		boolean posPos = false;
		Elements ngElements = null; // список элементов <n-g>
		/*
		 * список значений <dr-g> это случай когда adj. находится в noun
		 * entry22273.xml, mainstream, [C.false], posList: {adjective=1, verb=2,
		 * noun=0} 22.04.08
		 */
		...
		if (pg != null) { // 1.1. если есть pg значит несклько pos
			// if (pg != null || drg != null) { //1.1. если есть pg значит
			// несклько pos
			OALD.display("1.1. '" + pos + "' pg=" + pg); // + " drg=" + drg);
			// Elements entryChildren = entry.getChildElements("p-g"); //берем
			// список p-g
			Elements entryChildren = entry.getChildElements(); // берем список
																// p-g, dr-g
			for (int i = 0; i < entryChildren.size(); i++) { // ищем pos
				...
				if (el.getQualifiedName().equalsIgnoreCase("p-g")) {
					...
					if (pgPosList.get(pos) != null) {
						if (isEmptyGroup(getGroupList()))
							setGroupList(Entry.findGroupList(el
									.getChildElements("span")));
						// OALD.display("type=" + typeList);
						// 1
						// ngElements = el.getChildElements("n-g"); //берем
						// список значений <n-g>
						// if (ngElements.size() == 0) sdgElements =
						// el.getChildElements("sd-g"); //берем список значений
						// <sd-g>, если <n-g> внутри <sd-g>
						// OALD.writeLog("INFO: p-g - (" + pos + ") " +
						// toString() + " i=" + i
						...
						// 2
						// sdgElements = el.getChildElements("sd-g"); //берем
						// список значений <sd-g>, если <n-g> внутри <sd-g>
						// if (sdgElements.size() == 0) ngElements = el
						// .getChildElements("n-g"); //берем список значений
						// <n-g>
						// 3
						ngElements = el.getChildElements("n-g"); // берем список
																	// значений
																	// <n-g>
						// drgElements = el.getChildElements("dr-g"); //берем
						// список значений <dr-g> entry22273.xml, mainstream,
						// [C.false], posList: {adjective=1, verb=2, noun=0}
						sdgElements = el.getChildElements("sd-g"); // берем
																	// список
																	// значений
																	// <sd-g>,
																	...
																	// внутри
																	// <sd-g>
						OALD.writeLog("INFO: p-g.1 - (" + pos + ") "
								+ toString() + " i=" + i + " ngElements="
								+ ngElements.size()
								// + " drgElements=" + drgElements.size()
								+ " sdgElements=" + sdgElements.size());
						// OALD.display("1.1. ok '" + pos + "' ngElements="+
						// ngElements.size() + " sdgElements=" +
						// sdgElements.size());
						break;
					}
				}
			...
			if (ngElements == null && posList.size() == 1) { // entry07974.xml
				ngElements = pg.getChildElements("n-g"); // берем список
															// значений <n-g>
				// drgElements = pg.getChildElements("dr-g"); //берем список
				// значений <dr-g> entry22273.xml, mainstream, [C.false],
				// posList: {adjective=1, verb=2, noun=0}
				sdgElements = pg.getChildElements("sd-g"); // берем список
															// значений <sd-g>,
															// если <n-g> внутри
															// <sd-g>
			...
			if (posList.get(pos) != null) {
				ngElements = hg.getChildElements("n-g"); // берем список
															// значений <n-g>
				// drgElements = hg.getChildElements("dr-g"); //берем список
				// значений <dr-g> entry22273.xml, mainstream, [C.false],
				// posList: {adjective=1, verb=2, noun=0}
				sdgElements = hg.getChildElements("sd-g"); // берем список
															// значений <sd-g>,
															// если <n-g> внутри
															// <sd-g>
			}
		...
		if (ngElements == null) {
			if (POSNOUN.equalsIgnoreCase(pos)) {
				...
				OALD.writeEntry(this, Entry.GROUPERR);
			} else {
			...
			if (sdgElements != null && sdgElements.size() > 0) {
				...
				for (int i = 0; i < sdgElements.size(); i++) {
					...
					// +posPos =
					// analyzeNG(sdgElements.get(i).getChildElements("n-g"),
					// pos);
				}
			}
		}
		OALD.display("end analyzePG0(" + pos + ")=" + posPos);
		return posPos;
	}
